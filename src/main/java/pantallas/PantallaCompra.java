...
/**
 * Pantalla para crear un nueva compra
 * 
 * @author AlexLopez
 *
 */
public class PantallaCompra extends JPanel {
	...
	public PantallaCompra(Ventana v) {
		...
		/**
		 * Boton para registar la Compra introducido por los campos
		 */
		...
		Rejistrar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				try {
					...
					LocalDate fecha = LocalDate.of(Integer.parseInt(fechaPartida[2]), Integer.parseInt(fechaPartida[1]),
					...
					/**
					 * creamos una nueva compra y le pasamos los parmetros recurados si todo esta
		...
		add(Rejistrar);
/**
 * boton q nos pasa a la pantalla listado de compras y ademas nos crea el informe con funcion imprime compra
 */
